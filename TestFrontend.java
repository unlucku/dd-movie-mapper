...
public class TestFrontend {
	...
	public void runTests() {
		...
		System.out.print("Tests select/deselect feature on ratings (WARNING: if pressing 'x' twice from the rating screen does not exit app, test won't exit and run idefinitely!): ");
		if (this.testFrontendSelectDeselect()) {
			System.out.println("PASSED");
		} else {
			System.out.println("FAILED");
		}	
	...
	public boolean testFrontendRForRatings() {
		...
		try {
			...
			String appOutput = outputStreamCaptor.toString();
			for (int i = 0; i < 10; i++) {
				if (!(frontend != null && appOutput.contains(String.valueOf(i+1)))) return false;
			}
			return true;
		} catch (Exception e) {
			// make sure stdin and stdout are set correctly after we get exception in test
	...
	/**
	 * Tests the select/deselect functionality
	 *
	 * @return true if test passes, false otherwise
	 */
	public boolean testFrontendSelectDeselect() {
		PrintStream standardOut = System.out;
		InputStream standardIn = System.in;
		try {
			// set the input stream to our input (with an g to test of the program lists genres)
			String input = "r" + System.lineSeparator() + "1" + System.lineSeparator() + "x" + System.lineSeparator() + "x";
			InputStream inputStreamSimulator = new ByteArrayInputStream(input.getBytes());
			System.setIn(inputStreamSimulator);
			ByteArrayOutputStream outputStreamCaptor = new ByteArrayOutputStream();
			// set the output to the stream captor to read the output of the front end
			...
			if (frontend != null && appOutput.contains("SELECTED")) {
				// test passes
				return true;
			} else {
				// test fails
